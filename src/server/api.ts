...
export async function startApiServer(port: number = 3000) {
    ...
    server.listen(port, () => {
        console.log(`\n🌐 ${CLI_BRANDING.brand_name} API Server running at http://localhost:${port}`);
...
async function getSkillDetailPage(slug: string): Promise<string | null> {
    ...
    const skillSlug = generateSlug(skill.name || skill.id || '');
    const installCommand = `npx ${CLI_BRANDING.brand_lower_name} install ${skill.id || skill.name}`;
...
<head>
    ...
    <title>${skill.name} - ${CLI_BRANDING.brand_name}</title>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>404 - Page Not Found | ${CLI_BRANDING.brand_name}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${CLI_BRANDING.brand_name} - Universal AI Skills Marketplace</title>
