...
export async function toggleSkill(skillName: string, enable: boolean): Promise<void> {
    ...
    if (enable) {
        ...
        if (fs.existsSync(skillMdPath)) {
            ...
            console.log(chalk.gray(`   Re-enable with: npx ${CLI_BRANDING.brand_lower_name} enable ${skill.name}`));
...
export async function listDisabledSkills(): Promise<void> {
    ...
    disabled.forEach(skill => {
        ...
        console.log(chalk.gray(`     Enable: npx ${CLI_BRANDING.brand_lower_name} enable ${skill.name}`));
        console.log('');
    });
}
...
export async function interactiveToggle(): Promise<void> {
    ...
    if (enabled.length === 0 && disabled.length === 0) {
        ...
        console.log(chalk.gray(`Install skills with: npx ${CLI_BRANDING.brand_lower_name} install <skill-name>`));
        return;
    }
    ...
    console.log(chalk.gray(`Run \`npx ${CLI_BRANDING.brand_lower_name} sync\` to update AGENTS.md\n`));
}
