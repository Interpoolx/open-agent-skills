...
const DEFAULT_CONFIG: ConfigValue = {
    ...
    autoSync: false,
    registryUrl: CLI_BRANDING.fallback_url,
    cacheEnabled: true,
    cacheTTL: 24,
    defaultAgent: 'auto',
    confirmPrompts: true,
...
function getConfigPath(): string {
    return path.join(os.homedir(), `.${CLI_BRANDING.brand_lower_name}`, 'config.json');
}
    ...
    switch (action) {
        ...
        case 'set':
            if (!key || value === undefined) {
                console.log(chalk.red(`❌ Usage: npx ${CLI_BRANDING.brand_lower_name} config set <key> <value>`));
                showAvailableKeys();
                return;
            }
        ...
        default:
            console.log(chalk.cyan.bold(`\n⚙️  ${CLI_BRANDING.brand_name} Configuration\n`));
            console.log(chalk.bold('Usage:'));
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} config list        Show all settings`));
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} config get <key>   Get a specific value`));
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} config set <k> <v> Set a value`));
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} config reset       Reset to defaults`));
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} config edit        Interactive editor`));
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} config path        Show config file path`));
            console.log('');
            showAvailableKeys();
    }
}
