...
export async function syncAgents(options: SyncOptions): Promise<void> {
    ...
    if (skills.length === 0) {
        ...
        console.log(chalk.gray(`Install some skills first with: npx ${CLI_BRANDING.brand_lower_name} install <skill-name>`));
        return;
    }
...
How to use skills:
- Invoke: \`npx ${CLI_BRANDING.brand_lower_name} read <skill-name>\` (run in your shell)
- For multiple: \`npx ${CLI_BRANDING.brand_lower_name} read skill-one,skill-two\`
