...
export async function importFromManifest(manifestPath?: string, options: { force?: boolean; yes?: boolean } = {}): Promise<void> {
    ...
    const possiblePaths = manifestPath
        ? [manifestPath]
        : ['skills.json', `${CLI_BRANDING.brand_lower_name}.json`, `.${CLI_BRANDING.brand_lower_name}/skills.json`];
    ...
    if (!foundPath) {
        ...
        console.log(chalk.gray(`Searched for: skills.json, ${CLI_BRANDING.brand_lower_name}.json, .${CLI_BRANDING.brand_lower_name}/skills.json`));
        console.log(chalk.gray(`Or specify a path: npx ${CLI_BRANDING.brand_lower_name} import <path>`));
        return;
    }
    ...
    if (successCount > 0) {
        console.log(chalk.gray(`\n💡 Run \`npx ${CLI_BRANDING.brand_lower_name} sync\` to update AGENTS.md`));
    }
}
...
export async function generateManifest(outputPath?: string): Promise<void> {
    ...
    console.log(chalk.gray(`   npx ${CLI_BRANDING.brand_lower_name} import skills.json`));
}
