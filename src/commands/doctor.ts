...
export async function runDoctor(options: DoctorOptions = {}): Promise<void> {
    ...
    console.log(chalk.cyan.bold(`\n🩺 ${CLI_BRANDING.brand_name} Doctor\n`));
...
function printEnvironmentSummary(result: DetectionResult): void {
    ...
    const globalConfigPath = path.join(os.homedir(), `.${CLI_BRANDING.brand_lower_name}`, 'config.json');
    ...
    const cachePath = path.join(os.homedir(), `.${CLI_BRANDING.brand_lower_name}`, 'registry_cache.json');
...
async function runHealthChecks(options: DoctorOptions): Promise<void> {
    ...
    try {
        const testPath = path.join(process.cwd(), `.${CLI_BRANDING.brand_lower_name}-test-write`);
...
async function autoFix(result: DetectionResult): Promise<void> {
    ...
    if (!result.hasAgentsMd && result.installedSkillsCount > 0) {
        console.log(chalk.yellow(`  ⚠️  Run 'npx ${CLI_BRANDING.brand_lower_name} sync' to create AGENTS.md`));
    }

    // Fix 3: Create global config directory
    const globalDir = path.join(os.homedir(), `.${CLI_BRANDING.brand_lower_name}`);
