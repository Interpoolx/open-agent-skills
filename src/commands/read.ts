...
export async function readSkills(skillNames: string[]): Promise<void> {
    ...
    if (names.length === 0) {
        ...
        console.error(chalk.gray(`Usage: npx ${CLI_BRANDING.brand_lower_name} read <skill-name> [skill-name-2...]`));
        console.error(chalk.gray(`   or: npx ${CLI_BRANDING.brand_lower_name} read "skill-one,skill-two"`));
        process.exit(1);
    }
    ...
    if (results.notFound.length > 0) {
        ...
        console.log(chalk.gray(`Run 'npx ${CLI_BRANDING.brand_lower_name} list' to see installed skills.`));
    }
