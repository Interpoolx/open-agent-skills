...
export async function generateLockFile(options: { output?: string } = {}): Promise<void> {
    ...
    if (skills.length === 0) {
        ...
        console.log(chalk.gray(`Install skills first with: npx ${CLI_BRANDING.brand_lower_name} install <skill-name>`));
        return;
    }
...
export async function restoreFromLockFile(lockFilePath?: string): Promise<void> {
    ...
    if (!fs.existsSync(targetPath)) {
        ...
        console.log(chalk.gray(`Generate one with: npx ${CLI_BRANDING.brand_lower_name} lock`));
        return;
    }
...
export async function checkLockFile(): Promise<void> {
    ...
    if (added.length === 0 && removed.length === 0) {
        ...
        console.log('');
        console.log(chalk.gray(`Run \`npx ${CLI_BRANDING.brand_lower_name} lock\` to update the lock file.`));
    }
}
...
export async function manageLock(action?: string, lockPath?: string): Promise<void> {
    switch (action) {
        ...
        default:
            ...
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} lock            Generate lock file`));
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} lock restore    Restore from lock file`));
            console.log(chalk.gray(`  npx ${CLI_BRANDING.brand_lower_name} lock check      Check lock file status`));
            console.log('');
    }
}
